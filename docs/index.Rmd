...
healthboard_shp <- st_read(here("data", "SG_NHS_HealthBoards_2019.shp")) %>% 
  clean_names() %>% 
  rename(hb = hb_code)
...
ggplot() +
  ...
  scale_color_gradient(low = "#D20B0B",
                       high = "#F3DBD8",
                       name = "SIMD Decile") +
    labs(
...
Healthboards with higher average precriptions (EG) appear to have a a high number of GP Practices with a high SIMD. Additonally, healthboard with a lower averahge number of presctiptions (EG) have a significant number of GP Practices with a lower SIMD score. Along with the previous findings in Figure 1, this would suggest that SIMD is affecting the number of prescriptions for preventative inhalers on a smaller geographical level. This would be logical as even neighboring datazones can have drastically different SIMD scores. Together, this indicates that individual living standards really could be having a significant impact on the prevalence chronic asthma.
